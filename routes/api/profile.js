...
router.get(
    ...
    (req, res) => {
        ...
        Profile.findOne({user: req.user.id})
            .populate('user', ['name', 'avatar']) // now in profile below - user will have fields name and avatar.
            .then(profile => {
                if (!profile) {
...
router.get('/all', (req, res) => {
    ...
    Profile.find()
        ...
        .catch(err => res.status(404).json({profile: 'NO PROFILE MAN'}));
});
...
// @desc   Create or edit users profie
// @access Private
router.post(
    '/',
    passport.authenticate('jwt', {session: false}),
